...
【译者注：本文为《Advanced HD Video Encoding Guide》翻译，限制于译者压制与翻译水平均有限，翻译可能产生偏差甚至误导，在此表示歉意。如遇不明之处还请[查看原文](https://github.com/typing-more/encode_guide/raw/master/encode_guide_English.pdf)。中文版来自[这里](https://blog.cfandora.com/archives/211/)】
【译者再注：由于所有图片所在图床一起随时间而逝，所有附图均由PDF中提取再经由Photoshop拼接，过程中可能造成些许损失，但应该不影响区分。】

markdown版编者注：对排版与名词翻译进行了修订，找回了丢失的原图并替换进文内。

[Introduction - Advanced Encoding Guide (silentaperture.gitlab.io)](https://silentaperture.gitlab.io/mdbook-guide/)

## 1 工具

### 1.1 eac3to

虽然我们不喜欢推荐闭源软件，但不可否认的是，在没有其他足以替代的分流工具情况下，[eac3to](https://forum.doom9.org/showthread.php?t=125966) 是一个非常有用的工具。如果你不想下载闭源软件（更不用说支持它了），你也可以简单地使用 [MKVToolNix](https://mkvtoolnix.download/)，而不需要单独的解流器。
...
- [VapourSynth Editor](https://bitbucket.org/mystery_keeper/vapoursynth-editor/src/master/) 更推荐[MOD版](https://github.com/YomikoR/VapourSynth-Editor/releases/)
- [Yuuno](https://yuuno.encode.moe/)
- [VapourSynth Preview](https://github.com/Endilll/vapoursynth-preview/)
...
对于分流工作，有一个基于 ffmpeg 的优秀工具： [bdinfo](https://github.com/schnusch/bdinfo)。尽管它与另一个程序BDInfo同名，但它不能生成许多站点发布所需的蓝光碟片扫描信息。
...
​                                                          ![yuv420](Pictures/yuv420.png)                 ![yuv444](Pictures/yuv444.png)
...
![yuv420vsyuv444](Pictures/yuv420vsyuv444.png)
...
关于去色带，需要注意的一点是，你应该尽量使用遮罩，最好是边缘遮罩或类似的东西。详见[3.2.14](https://blog.cfandora.com/archives/211/####3.2.14))！
...
- `grainy`/`grainc` 为了防止带状物再次出现，并抵消平滑度，通常在去色带过程后加入**颗粒(grain)**。然而，由于这种假颗粒是相当明显的，建议保守一点添加。另外，你可以使用自定义加噪，这将得到一个更好的输出（见[3.2.10](https://blog.cfandora.com/archives/211/####3.2.10))）
...
​                      ![banding_before3](Pictures/banding_before3.png)           ![banding_after3](Pictures/banding_after3.png)
...
如果去色带处理后的片段与未经处理的相比噪点很少，此时你应该考虑使用一个单独的函数来添加匹配的噪点，这样场景就更容易融合在一起。 如果有很多噪点，你可能要考虑使用 `adptvgrnMod`、`adaptive_grain` 或 `GrainFactory3`；对于不太明显的噪点或对于通常只有很少噪点的明亮场景，你也可以使用 `grain.Add`。这个话题将在后面的[3.2.10](https://blog.cfandora.com/archives/211/####3.2.10))中进一步阐述。

![banding_graining](Pictures/banding_graining.png)

图4：左边是源，右边是滤镜处理后的。这里的色带可能很难发现，但我不能在这个PDF中加入更大的图片。不过，这个效果应该是显而易见的。

如果你想进行自动色带检测，你可以使用一个基于 `bandmask` 的检测函数，叫做 `banddtct`。确保适当调整数值并检查全部输出。[3.2.17](https://blog.cfandora.com/archives/211/####3.2.17)中有一个解释它的论坛帖子链接。你也可以在运行 `adptvgrnMod` 或`adaptive_grain` 时使用一个高的 `luma_scaling` 值，希望噪点能完全覆盖它。 更多关于这个的内容在[3.2.10](https://blog.cfandora.com/archives/211/####3.2.10))中继续讨论。
...
![improper_borders](Pictures/improper_borders.png)
...
图6：来源于的D-Z0N3的压制作品《声之形》（A Slinet Voice (2016)）的脏线修复与过滤。在最上面的三行使用 `ContinuityFixer`，在最左边的两列使用`FillBorders`。 当前画面放大15倍。
...
![bilinear_before2](Pictures/bilinear_before2.png)

![bilinear_after2](Pictures/bilinear_after2.png)
...
![matrix_conversion](Pictures/matrix_conversion.png)
...
<center>
<img src="Pictures/gradfun3.png"  style="zoom: 25%;"/><img src="Pictures/debandmask.png"  style="zoom: 25%;" />
<img src="Pictures/retinex.png"  style="zoom: 25%;" /><img src="Pictures/gradfun3.png"  style="zoom: 25%;" />
</center>
...
```mermaid
graph TD;
	A[载入源滤镜 Load Source]-->B[切黑边 Crop our];
	B-->C[抖动至高位深 High bit depth];
	C-->D[反交错与MPEG2去色块 IVTC and MPEG-2 deblock];
	D-->E[去脏线 Fix dirty lines];
	E-->F[色彩校正 Detint];
	F-->G[缩放 Resize or descale];
	G-->H[降噪 Denoise];
	H-->I[去光晕与去振铃 Dehalo and Dering];
	I-->J[抗锯齿 Anti-Aliasing];
	J-->K[去色带与去色块 Deband and deblock];
	K-->L[颗粒化 Grain];
	L-->M[降到输出位深 Dither to output depth];
```
...
- https://github.com/Beatrice-Raws/encode-scripts Various scripts by BeatriceRaws
...
- 对何时与如何使用脏线修复的看法（使用ContinuityFixer）：https://passthepopcorn.me/forums.php?action=viewthread&threadid=35149&postid=1656050#post165605038h
...
由于硬件支持较差，这种编解码器不如 DTS-HD MA 流行，但它的压缩率更高一些，特别是在低比特率的情况下，DTS核心的臃肿并不会使比特率膨胀。遗憾的是，由于其较差的硬件兼容性，很多小组坚持再封装一条单独的AC-3兼容音轨。在Concertos(国外的一家音乐视频类PT)中，我们希望您不要这样做，但我们理解这可能是上传其他地方所必须的，或者仅仅是您为了硬件兼容性所做的妥协。
...
- Irrational Encoding Wizardry’s fansubbing guide: [https://guide.encode.moe](https://guide.encode.moe/)
...
![resizers_down](Pictures/resizers_down.png)

图16：不同缩小像素函数的效果比较。将源从1080p缩小到720p，然后被裁剪。所有参数都保留默认值。

![resizers_up](Pictures/resizers_up.png)

图17：放大以后的大小。来自于Commie压制的《你的名字》（Your Name (2016)），相较于源进行了裁剪，然后被放大了2倍。

![bicubic_params](Pictures/bicubic_params.png)

图18：不同参数的双三次(bicubic)放大比较。记住，这些只是视频的小片段，双三次放大总是会导致伪影，特别是锯齿。尖锐的双三次曲线(b=0, c=1)特别容易产生锯齿伪影。

![resize_downup](Pictures/resize_downup.png)

图19：缩小并放大到原始分辨率的调整器比较。所有内容都被缩小到源分辨率的一半，然后使用`nnedi3_rpow2`放大回去。所有参数都保留默认值。

![resize_downup2](Pictures/resize_downup2.png)

图20：缩小并放大到原始分辨率的调整器比较。所有内容都被缩小到源分辨率的一半，然后用相同的调整器放大回去。所有参数都保留默认值。

![dither_comparison_8](Pictures/dither_comparison_8.png)

图21：抖动类型的比较。输入源为8-bit视频，向上抖动并用 `f3kdb` 去色带，输出为16-bit，然后抖动至8-bit。

![improper_improper_borders](Pictures/improper_improper_borders.png)

图22：一个说明为什么不适当的对角边界fx是个坏主意的例子。上面是源，下面是Geek使用边缘遮罩后的。来自Geek压制的《精灵旅社2》（Hotel Transylvania 2 (2015))。

![hbd_example](Pictures/hbd_example.png)

图23：在16-bit（右)和8-bit（左)分别进行伽马修复(gamma bug fix)的比较

<center>
<img src="Pictures/edgemasks_src.png"  style="zoom: 75%;" /><img src="Pictures/edgemasks_prewitt.png"  style="zoom: 75%;" />
<img src="Pictures/edgemasks_sobel.png"  style="zoom: 75%;" /><img src="Pictures/edgemasks_kirsch2.png"  style="zoom: 75%;" />
<img src="Pictures/edgemasks_tcanny.png"  style="zoom: 75%;" /><img src="Pictures/edgemasks_retinex.png"  style="zoom: 75%;" />
</center>
...
<center>
<img src="Pictures/edgemasks_src2.png"  style="zoom: 25%;" /><img src="Pictures/edgemasks_retinex2.png"  style="zoom: 25%;" />
<img src="Pictures/edgemasks_kirsch.png"  style="zoom: 25%;" /><img src="Pictures/edgemasks_fastsobel2.png"  style="zoom: 25%;" />
</center>
...
![masked_debanding](Pictures/masked_debanding.png)
...
本文翻译主要基于[DeepL翻译器](https://www.deepl.com/translator)，在此基础上大量参考[typing-more/高清视频转码进阶指南.md](https://github.com/typing-more/encode_guide/blob/master/高清视频转码进阶指南.md)，在此表示由衷感谢！
